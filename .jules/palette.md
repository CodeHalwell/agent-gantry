...
## 2026-08-30 - ARIA Tablist Focus Management
**Learning:** When implementing keyboard navigation for ARIA tablists in React, merely updating the active state index via keyboard handlers is insufficient. You must use DOM refs to track the tab elements and programmatically call .focus() on the newly activated tab, ensuring focus is not trapped.
**Action:** Always use DOM refs and apply tabIndex={0} to the active tab and tabIndex={-1} to inactive tabs alongside ArrowRight/ArrowLeft keypress handlers.
